...
class Scene {
protected:
    std::function<void()> onReturnHome = nullptr;

public:
    void setHomeCallback(std::function<void()> cb) { onReturnHome = cb; }
...
public:
    IntroScene(Resource* r, PlayerType t, std::function<void()> callback);
    void render() override;
    void handleInput(ExMessage msg) override;
};
